...
- ✅ **已完成：Phase L.4 — DSH runtime facade 精简 (-2324 LOC)**（commit `b22fd17`, task LUM-595）：`electron/main/deepseek/deepseek-runtime.ts` 从 4368 → 3827 LOC（-541，删除 DeepSeekLlmService + DeepSeekTypertLoaderService + DeepSeekTypertGatewayService + DeepSeekAgentLoopService + DeepSeekAgentDefaultModelService + createDeepSeekPiAgentLoopPlugin + deepSeekSessionQueryRemote + 内部 gateway / agent-factory helpers）；删除 `electron/main/deepseek/dsh-host-runner.ts` (43) + `electron/main/deepseek/deepseek-execution-adapters.ts` (69) + `electron/main/agent/host-modules/facade/deepseek-facade.ts` (44) + `electron/main/deepseek/deepseek-agentloop-pi-smoke.test.ts` (833)；`wire-dsh-services.ts` 313 → 187 LOC，仅保留 goal + message-feedback 状态机；`init-deepseek.ts` 189 → 84 LOC，移除 7-capability remote 注册 + reconcileProfileArtifacts 重注册；**总计 -2324 LOC 退役**
- ✅ **已完成：Phase K.1 — OpenBuddyPlugin SDK v0.1 独立包**（commit `04f41fe`）：新建 `packages/runtime/openbuddy-plugin-sdk/` 包（825 LOC production + 24 tests + README），zod schema 解析 `plugin.json` / `package.json#openbuddy`，输出 PI ExtensionFactory；sample-plugin fixture（pi/harness/cordis/slot 4 轨 manifest）+ 4 个单测（manifest/serializer/index/fixtures）；**LOC 净增 +825**
- ✅ **已完成：Phase K.2 — OpenBuddyPlugin SDK 接入 builtin extension / harness / slot**（commit `0e7f354`, task LUM-597）：
  - **`openbuddy-plugin-host` K.1 manifest 路径**：Zod schema `openbuddy.plugin.v1` + 4 个 single-track serializers (serializePiTrack / serializeHarnessTrack / serializeSlotTrack / serializeCordisTrack) + passthrough helper + 21 个单测。后续 v8 commit `b602149` 把 manifest 路径独立成 `packages/runtime/openbuddy-plugin-sdk/`（v4 §25 spec 落地）。
  - **`pi-extensions.ts` 改造**：9 个 builtin extension factories 改为 OpenBuddyPlugin manifest 格式（serialize PiTrack → PI ExtensionFactory）+ 5 个 manifest 单测。
  - **`init-deepseek.ts` 接入 SDK**：用 `serializeHarnessTrack` 投影 coreCapabilityManifest 到 HarnessPluginLoader 入口 + 4 个单测。
  - **`plugin-ui-host.ts` 改造**（即 `packages/ui/openbuddy-ui-runtime/src/slot-plugin-manifest.ts`）：新增 `BuiltinUiPluginSlotTrack` 类型 + `toOpenBuddyPluginManifest()` + `serializeBuiltinUiSlotTrack()`。`BUILTIN_UI_APPLIES` 表新增 description 字段，注释里描述 Phase K.2 投影路径。`registerAllBuiltinUis()` 在每次 `apply()` 前调用 SDK 序列化（manifest-level validation 提前 surface，runtime 实际装载走 in-process apply 不变）。新增 5 个单测覆盖 manifest 投影。
  - **`serializeAllTracks` aggregate helper**（commit `d055087`）：单次返回 `{ id, pi, harness, slot, cordis, passthroughApplied }`，L.3 step 2 删除 `deepseek-compat.ts` 后 init-deepseek.ts 走这个 seam。
  - **`serializeRemoteContribution`**（commit `cba342b`）：K.2 的 harness remote contribution 序列化辅助。
  - **`openbuddy-plugin-host/src/index.ts` re-exports**：`serializeHarnessTrack` + `serializePiTrack` + `serializeSlotTrack` + `serializeAllTracks` 等公共 API。
  - **`__fixtures__/sample-plugin/`**：作为 K.1 SDK 输出的 reference。包含 `plugin.json` (4 track manifest: pi/harness/cordis/slot)、`package.json` (openbuddy.bundle + openbuddy.client + pi.extensions/skills)、`index.js` (cordis plugin 默认导出)、`extensions/index.js` (PI ExtensionFactory)、`client.js` (slot apply)、`remote.js` (remote contribution)、`cordis.patch.yml` (harness patch)、`skills/sample-plugin/SKILL.md`、5 个 round-trip 单测。
  - **测试净增量**: `+38 个新单测`（K.1 SDK 21 + builtin pi manifest 5 + init-deepseek 4 + slot-plugin-manifest 5 + sample-plugin fixture 5 - 与 `__fixtures__/external-dsh-plugin/` 互不干扰）。
  - **LOC 净增**: `+761/-42 = +719`（SDK 352 + pi-extensions 184 + builtin-applies 87 + slot-plugin-manifest 67 + init-deepseek 68 + client.tsx 20 + tests + fixture file docs）。这是必要的成本：v6 §3.4 要求 SDK 是一个独立模块而不是埋在 pi-extensions.ts 里。
  - **K.2 不完成 DSH 装载入口的替换**：`loader.loadProfile(profile)` 在 `init-deepseek.ts` 保留（HarnessPluginLoader 路径仍在用）；真正切到 PI `loadExtensions()` 是 Phase L.3 的责任（v6 §24.4 表）。K.2 只把 manifest 形状统一到 `openbuddy.plugin.v1`，实际装载不在 K.2 范围。
- ✅ **已完成：Phase L.3 step 1 — `deepseek-generic.ts` 极简化（-1787 LOC net）**（本轮 commit, task LUM-600）：`deepseek-generic.ts` 从 1545 → 103 LOC（删 createSettingsService / createCredentialsService / createSystemPromptService / createWorkflowEngineService / createUserQuestionsService / createAgentInstructionsService / createAgentPresetsService / createApprovalService / concretePlanTools / concreteWebTools / concreteDelegationTools / concretePiTools / concreteAskUserTools / concreteSessionQueryTools / concreteEditorTools / concreteSkillTools / concreteWorkflowTools / concreteTodoTools / concreteGoalTools / terminalTools / genericPlugin / genericRemote / genericExport / HostJobs / jobsFor / serviceKeyFor / sharedServiceKeys / delegateKeys / explicitDeepSeekPackages 等所有 DSH service 实现 + Proxy 兼容层）；`deepseek-generic.test.ts` 从 446 → 112 LOC（删 10 个 DSH-specific loader/smoke 集成测试，保留 13 个 shim API 单测）；`resolveDeepSeekGenericModule` 改成 no-op plugin shim（`apply: async () => () => undefined`），让 `HarnessPluginLoader.normalizePlugin()` 在 `node_modules/@deepseek-ai/` 空场景下不 throw；**总计 -1787 LOC 退役**（171 insertions / 1958 deletions 跨 2 个文件）。
- 🟡 **下一轮 — Phase L.3 step 2 — `deepseek-compat.ts` 删除（-445 LOC + 14 调用方迁移）**（v5 §26.4 + v7 §11.2）：把 `deepseek-compat.ts` (445) + `deepseek-compat.test.ts` (2039) 整文件删除；`resolveDeepSeekModule` 改成 `init-plugin-loader.ts` inline stub 返回 undefined；14 调用方（init-pipeline-builder / init-pipeline / init-plugin-loader / agent-host / cordis-runtime / workbench-scope / packages/runtime/openbuddy-plugin-host/src/include.ts 等）走 PI `loadExtensions()` 路径。预期 **-2484 LOC 退役**（-445 + -2039）。
- ⚪ **下一轮（本任务 LUM-600 三期） — Phase L.3 step 3 — `deepseek-agentloop-pi-smoke.test.ts` 改名 + PI-specific 抽取**（v5 §26.4）：833 LOC 测试提取 PI-specific subtests 到新文件 `electron/main/agent/__tests__/agent-loop-pi-smoke.test.ts`（~400 LOC）；剩余 DSH-specific 部分删除。预期 **-433 LOC 退役**。
- ⚪ **下一轮（本任务 LUM-600 三期） — Phase L.4 已完成（DSH runtime facade 精简）**（commit `b22fd17`，已并入 L.4 + K.2 完成表）。
- ⚪ **下一轮（本任务 LUM-600 三期） — Phase J.1（部分）**：先跑 `pnpm storage:boundaries` + `pnpm storage:acceptance` 拿到当前 baseline，然后加固 sheriff.config.ts（覆盖 §3.2 的 4 层依赖方向 + §4 的 god module 拆解 + §5 的 6 类耦合点 0 处）
