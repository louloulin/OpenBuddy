...
- ✅ **已完成：Phase K.1 — OpenBuddyPlugin SDK v0.1**（本轮 commit）：新建 `packages/runtime/openbuddy-plugin-sdk/` (~450 LOC + 24 单测)；SDK 按 v6 重定位为 **PI ExtensionFactory 的 manifest 序列化辅助**（`plugin.json` + `package.json#openbuddy` → zod 校验 → PI `ExtensionFactory`），实际装载仍走 PI `loadExtensions()`；4 轨（pi / cordis / ui / harness）类型保留，cordis 轨运行时再装载；vitest 24/24 pass + `moon run packages/runtime/openbuddy-plugin-sdk:{typecheck,test}` 全绿 + 邻近 runtime 包（plugin-host / cordis）回归 0；SDK README 说明 PI-native manifest 规范
