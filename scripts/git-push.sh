...
#   LegacyGeneric:target=git:https://louloulin@github.com
#
# Important: keep the userinfo (louloulin@). A bare
# https://github.com/... URL is rewritten by global insteadOf to
# ghproxy.net and then authenticated push fails.
DEFAULT_PUSH_URL="https://louloulin@github.com/louloulin/OpenBuddy.git"
PUSH_URL="${GIT_PUSH_URL:-$DEFAULT_PUSH_URL}"

# Any URL that resolves to this repo is rewritten to the canonical
# authenticated form above. Matched case-insensitively and without
# anchoring the scheme, so ssh://, git@, ghproxy.net/, gh-proxy.com/ and
# the openbuddy/OpenBuddy casings all normalize to one URL.
normalize_push_url() {
  local url="$1"
  local lower
  lower="$(printf '%s' "$url" | tr '[:upper:]' '[:lower:]')"
  case "$lower" in
    *github.com/louloulin/openbuddy|*github.com/louloulin/openbuddy.git|\
    *github.com:louloulin/openbuddy|*github.com:louloulin/openbuddy.git)
      printf '%s\n' "$DEFAULT_PUSH_URL"
      ;;
    *)
