...
const userData = mkdtempSync(join(tmpdir(), "ob-dk-"));
...
await page.evaluate(() => {
  ...
  try { localStorage.setItem('openbuddy.theme', 'dark'); } catch {}
});
await page.waitForTimeout(1500);
await page.screenshot({ path: "/Users/louloulin/Downloads/home-dark-now.png" });
...
const m = await page.evaluate(() => {
  ...
  return {
    ...
    homePadding: cs('.home', 'padding'),
    titleColor: cs('.home__title', 'color'),
    heroPadTop: cs('.home__hero', 'paddingTop'),
    composerBg: cs('.wb-composer', 'backgroundColor'),
    practiceCardBg: cs('.home__practice-card', 'backgroundColor'),
    composerWrapBg: cs('.wb-composer-wrap--home', 'backgroundColor'),
  };
});
