...
const userData = mkdtempSync(join(tmpdir(), "ob-se2-"));
...
// Click settings (gear icon in sidebar footer) using locator
const settingsBtn = page.locator('button[aria-label="设置"]').first();
const cnt = await settingsBtn.count();
console.log('settings button count:', cnt);
if (cnt > 0) {
  await settingsBtn.click();
  await page.waitForTimeout(3000);
}

await page.screenshot({ path: "/Users/louloulin/Downloads/ob-r7-settings2.png", fullPage: false });
...
const m = await page.evaluate(() => {
  ...
  return {
    ...
    settingsPanel: b('.settings-panel, [class*="settings"]'),
    dialog: b('[role="dialog"], .modal'),
