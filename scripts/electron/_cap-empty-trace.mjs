...
const userData = mkdtempSync(join(tmpdir(), "ob-et-"));
...
await page.evaluate(() => {
  ...
  if (main) {
    ...
    div.innerHTML = '<span class="chatview__empty-state-tag">Test</span>';
...
const m = await page.evaluate(() => {
  ...
  // Check what bg-subtle resolves to
  const htmlCs = getComputedStyle(document.documentElement);
  return {
    bgComputed: cs.backgroundColor,
    bgProperty: cs.getPropertyValue('background-color'),
    htmlBgSubtle: htmlCs.getPropertyValue('--wb-bg-subtle'),
    htmlBgSecondary: htmlCs.getPropertyValue('--wb-bg-secondary'),
    htmlBgSubtleOnTag: cs.getPropertyValue('--wb-bg-subtle'),
  };
});
