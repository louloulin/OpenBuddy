...
<img alt="Tests" src="https://img.shields.io/badge/tests-3047%20files-success?style=flat-square&logo=vitest&logoColor=white">
...
> ```
> Without credentials the same script captures only `01-home-cold-start.png` and exits 0 with a warning, so low-privilege CI can still run it. The full seven-shot capture uses the same `.env.e2e.local` keys the e2e specs use (see `scripts/lib/e2e-credentials.mjs`).

### 1. Cold start — no API key configured

<p align="center">
  <img src="docs/screenshots/01-home-cold-start.png" alt="OpenBuddy cold start — sidebar with 任务 / 工作空间 / 设置, main area shows '正在本地初始化 agent…', composer disabled with a hint to configure an API key first" width="1024" />
</p>

### 2. Composer ready after configuring MiniMax

<p align="center">
  <img src="docs/screenshots/02-composer-ready.png" alt="After configuring the MiniMax-M3 provider the composer is enabled, the model selector is populated, the input is empty and ready for the first prompt" width="1024" />
</p>

### 3. Real MiniMax-M3 streaming answer — mid-turn

<p align="center">
  <img src="docs/screenshots/03-turn1-mid-stream.png" alt="Real MiniMax-M3 model streaming a self-introduction — the stop button is visible, the 深度思考 (deep thinking) block is collapsible, partial markdown is rendered inside the assistant bubble" width="1024" />
</p>

### 4. Turn 1 settled

<p align="center">
  <img src="docs/screenshots/04-turn1-settled.png" alt="First turn settled — status pill shows elapsed time, Buddy's reply is fully readable, copy / retry / thumbs-up actions visible below the message" width="1024" />
</p>

### 5. Turn 2 — multi-turn conversation

<p align="center">
  <img src="docs/screenshots/05-turn2-settled.png" alt="Second turn — user asks how to translate 'Hello world' to Chinese, Buddy's settled bubble shows the one-line Chinese answer above the new input row" width="1024" />
</p>

### 6. Turn 3 — structured output (Chinese to-do list)

<p align="center">
  <img src="docs/screenshots/06-turn3-settled.png" alt="Third turn — user asks for a three-line Chinese to-do list, Buddy outputs three short Chinese items as a rendered markdown list" width="1024" />
</p>

### 7. Full three-turn transcript

<p align="center">
  <img src="docs/screenshots/07-three-turns-full.png" alt="OpenBuddy AI chat full transcript of three turns — real MiniMax-M3 responses, each turn with a collapsible 深度思考 block, copy / retry / thumbs-up actions, message-meta token badges on assistant turns" width="1024" />
</p>
