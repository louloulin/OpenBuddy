...
> 版本：3.6 — 2026-09-10 Phase 7 完善轮
> 本轮实现：Phase 1 `SessionEventRecord.eventId` 校准 + Phase 5 reload listener leak gate（20 次 replace 无 listener 泄漏）+ Phase 0 capability inventory 本地生成
> 基线：`louloulin/OpenBuddy` checkout `5451ee2`（2026-09-10）+ 本轮修改
...
顺序一致性修复：remove/clear 重新归一化 order，避免删除后新任务与旧任务产生重复或跳号顺序。
...
3. 完成 Composer envelope、审批、取消、重试、暂停、恢复和 before-quit 策略（UI/IPC 侧待做）。
4. Artifact/Citation service 记录来源 task/session/tool/provider、版本和 hash（待做）。
5. SQLite 产品 metadata 与 Pi JSONL transcript 分层，完成迁移、冷启动、crash recovery、shadow compare（待做）。
...
本轮（v3.6，Phase 7 完善轮）已完成**Phase 1 `eventId` 校准 + Phase 5 reload listener leak gate + Phase 0 capability inventory 本地生成**（E1 Verified）。继续维护现有 Phase 2.1 task lifecycle owner / Phase 2.1 event audit log / Phase 2.1 worktask IPC / Phase 2.1 exit safety 在 origin/main 之前的 commit，已在本分支 rebase 后保留：

**已验证（E1）：**

| 结论                       | 证据                                                                                                                     | 位置                                           |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------- |
| Phase 7 inventory          | ✅ `evidence/pi-capability-inventory.json` 本地生成（10 builtin、12 adapter、5 runtime API、1 sessionEventHandlers、1 resourceLoaders） | `evidence/pi-capability-inventory.json`（gitignored — 本地产物） |
| eventId 字段补齐（v3.6）   | ✅ `SessionEventRecord.eventId?: string` + `randomUUID()`；每次 `appendFromSession` 必带 UUID，UUID 格式/唯一性/快照保留测试通过 | `electron/main/agent/pi-event-bridge.ts:21,155` |
| reload listener leak gate  | ✅ 14/14；20 次顺序 `runtime.replace()` 后 active subscription 仍为 1，每个 replaced session 仅 dispose/abort 1 次；unsubscribe 后 replace 新 session 为 0 subscribers | `electron/main/agent/pi-session-runtime.test.ts:197-235` |
| 本轮回归（4 套件）         | ✅ 24/24（pi-event-bridge 10 + pi-session-runtime 14）                                                                    | `vitest run --reporter=dot`                    |
| 既有 pi 套件回归           | ✅ pi-runtime-coordinator 5/5                                                                                            | `vitest run --reporter=dot`                    |
| typecheck                  | ✅ `tsc --noEmit -p tsconfig.json` 0 errors                                                                              | 直接 tsc                                       |

**已知约束：**

- `agent-host.ts` 1526 行 / `pi-extensions.ts` 1222 行（Phase 1.1/1.2 拆分目标尚未完成，仅有局部 host-modules/ facade 抽取；本轮未引入回归）
- 5 个文件 11 个 vitest 用例在 main 上 pre-existing fail（marketplace-pi-sync 等 3 用例、其它散点 8 用例），stash 我的 diff 后复测 11 个 fail 数与现象完全一致 — 本轮未引入新失败
- Electron smoke：❌ Linux 无 X server/Wayland（环境约束，preflight 正确报告 `desktopSmokeReady: false`）
- 真实 provider E3：⬜ 未执行（无凭据）
- 全量 vitest：⬜ 未执行（600+ files 超时，本轮仅跑关键 4 套件）
- moon typecheck：需要 Node ~22，环境仅有 Node 22；直接 tsc 无此约束
