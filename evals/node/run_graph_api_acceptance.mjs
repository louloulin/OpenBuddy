...
const enabled = process.env.OPENBUDDY_EMAIL_GRAPH_API_ACCEPTANCE === "1";
const token = String(process.env.OPENBUDDY_EMAIL_GRAPH_ACCESS_TOKEN ?? "").trim();
...
if (!enabled || !token) {
  console.error("Microsoft Graph acceptance is fail-closed: set OPENBUDDY_EMAIL_GRAPH_API_ACCEPTANCE=1 and a temporary OAuth access token.");
  process.exit(2);
}
...
if ((requested.write || requested.send) && !process.env.OPENBUDDY_EMAIL_GRAPH_TEST_RECIPIENT?.trim()) {
  console.error("Graph write/send mode requires OPENBUDDY_EMAIL_GRAPH_TEST_RECIPIENT.");
  process.exit(2);
}
...
const recipient = String(process.env.OPENBUDDY_EMAIL_GRAPH_TEST_RECIPIENT ?? "").trim();
const subject = `OpenBuddy Graph API acceptance ${Date.now().toString(36)}`;
const vite = await createServer({ configFile: path.resolve("vitest.config.ts"), server: { middlewareMode: true }, appType: "custom" });
const { MicrosoftGraphEmailProvider } = await vite.ssrLoadModule(path.resolve("packages/capability/openbuddy-email/src/microsoft-graph-provider.ts"));
const provider = new MicrosoftGraphEmailProvider({ accessToken: token, maxResults: 20 });
let account;
let firstPage;
let firstThread;
...
try {
  const accounts = await check("profile", async () => {
    ...
    if (!result[0]?.address || result[0].status !== "connected") throw new Error("Microsoft Graph profile is not connected");
    account = result[0];
  ...
  await check("provider-diagnostics", async () => {
    ...
    if (diagnostics.readiness !== "ready") throw new Error(`Graph provider readiness is ${diagnostics.readiness}`);
  ...
  firstPage = await check("thread-list-search", async () => {
    const page = await provider.threadsPage({ query: process.env.OPENBUDDY_EMAIL_GRAPH_QUERY?.trim() || "", limit: 10 });
  ...
  await check("pagination", async () => {
    ...
    const next = await provider.threadsPage({ query: process.env.OPENBUDDY_EMAIL_GRAPH_QUERY?.trim() || "", limit: 10, cursor: firstPage.nextCursor });
    if (next.nextCursor === firstPage.nextCursor) throw new Error("Graph returned a repeated nextLink");
    return { count: next.items.length, cursorAdvanced: true };
  }, Boolean(accounts));
  await check("folders-categories-and-thread", async () => {
    const labels = await provider.labels(account.id);
    if (!Array.isArray(labels) || labels.length === 0) throw new Error("Graph folders response is empty");
    if (firstThread) {
      firstThreadDetail = await provider.thread(account.id, firstThread.id);
      if (firstThreadDetail.id !== firstThread.id) throw new Error("Graph conversation identity mismatch");
    }
  ...
  await check("reversible-management", async () => {
    if (!firstThread) return { status: "not-run", reason: "no conversation available" };
    const before = await provider.thread(account.id, firstThread.id);
    const originalStarred = firstThread.starred === true;
    const originalUnread = before.messages.at(-1)?.unread === true;
    try {
      ...
      return { thread: digest(firstThread.id), operations: ["flag-roundtrip", "read-roundtrip"] };
    } finally {
      await provider.update({ accountId: account.id, threadId: firstThread.id, kind: "star", value: originalStarred });
  ...
  await check("draft-write-and-idempotent-update", async () => {
    const input = { accountId: account.id, to: [{ address: recipient }], subject, body: "OpenBuddy Graph API acceptance draft." };
    createdDraft = await provider.createDraft(input);
    const updated = await provider.createDraft({ ...input, draftId: createdDraft.id, body: "OpenBuddy Graph API acceptance draft updated." });
    if (updated.id !== createdDraft.id) throw new Error("Graph draft update returned a different message id");
    const visible = await provider.threadsPage({ folder: "drafts", query: subject, limit: 20 });
    if (visible.items.filter((item) => item.subject === subject).length !== 1) throw new Error("Graph Drafts did not show exactly one updated draft");
  ...
  await check("controlled-send-and-sent-visibility", async () => {
    ...
    throw new Error("sent message was not visible in Graph Sent Items");
  }, requested.send);
  await check("attachment-list-and-download", async () => {
    if (!firstThreadDetail) return { status: "not-run", reason: "no readable conversation" };
    const attachment = firstThreadDetail.messages.flatMap((message) => message.attachments).at(0);
    if (!attachment) return { status: "not-run", reason: "selected conversation has no attachment" };
    const listed = await provider.listAttachments(account.id, attachment.messageId);
    if (!listed.some((item) => item.id === attachment.id)) throw new Error("Graph attachment listing did not contain the selected attachment");
...
const report = { schema: "openbuddy.graph-api-acceptance.v1", evidenceLevel: "real-external", provider: "graph-api", requested, accountDomain: account?.address?.split("@")[1] ?? "unknown", checks, passed: checks.filter((item) => item.status === "passed").length, failed: failed.length, notRun: checks.filter((item) => item.status === "not-run").length };
if (evidenceDir) {
  mkdirSync(evidenceDir, { recursive: true });
  writeFileSync(path.join(evidenceDir, "graph-api-acceptance.json"), `${JSON.stringify(report, null, 2)}\n`, { mode: 0o600 });
}
console.log(JSON.stringify({ ...report, evidenceArtifact: evidenceDir ? path.join(evidenceDir, "graph-api-acceptance.json") : null }, null, 2));
