/**
 * Real e2e for `@remnic/plugin-pi` (G8 PR 2, plan4.1.md §9.16.8).
 *
 * Verifies that the canonical remnic plugin-pi package installs and exposes
 * a loadable entry. Listed in
 * `electron/main/agent/pi-extension-discovery.ts:23`.
 */
...
describe("canonical-pi: @remnic/plugin-pi real install (G8 PR 2)", () => {
  let result: InstallResult;

  beforeAll(() => {
    result = tryInstallCanonicalPiPackage("@remnic/plugin-pi", 120_000);
  }, 120_000);
  ...
  it("exposes a package.json with name and version", () => {
    if (result.specOnly) return;
    const meta = inspectInstalledPackage(result.cwd, "@remnic/plugin-pi");
    expect(meta.hasNodeModules).toBe(true);
    expect(meta.name).toBe("@remnic/plugin-pi");
