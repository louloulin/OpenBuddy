/**
 * Real e2e for `pi-schedule` (G8 PR 2, plan4.1.md §9.16.8).
 *
 * Verifies that the canonical schedule package installs and exposes
 * a loadable entry. Listed in
 * `electron/main/agent/pi-extension-discovery.ts:48`.
 */
...
describe("canonical-pi: pi-schedule real install (G8 PR 2)", () => {
  let result: InstallResult;

  beforeAll(() => {
    result = tryInstallCanonicalPiPackage("pi-schedule", 120_000);
  }, 120_000);
  ...
  it("exposes a package.json with name and version", () => {
    if (result.specOnly) return;
    const meta = inspectInstalledPackage(result.cwd, "pi-schedule");
    expect(meta.hasNodeModules).toBe(true);
    expect(meta.name).toBe("pi-schedule");
