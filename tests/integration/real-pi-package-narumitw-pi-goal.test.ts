/**
 * Real e2e for `@narumitw/pi-goal` (G8 PR 3, plan4.1.md §9.17.9).
 *
 * Verifies that the canonical narumitw goal package installs and exposes
 * a loadable entry. Listed in
 * `electron/main/agent/pi-extension-discovery.ts:44`.
 */
...
describe("canonical-pi: @narumitw/pi-goal real install (G8 PR 3)", () => {
  let result: InstallResult;

  beforeAll(() => {
    result = tryInstallCanonicalPiPackage("@narumitw/pi-goal", 120_000);
  }, 120_000);
  ...
  it("exposes a package.json with name and version", () => {
    if (result.specOnly) return;
    const meta = inspectInstalledPackage(result.cwd, "@narumitw/pi-goal");
    expect(meta.hasNodeModules).toBe(true);
    expect(meta.name).toBe("@narumitw/pi-goal");
