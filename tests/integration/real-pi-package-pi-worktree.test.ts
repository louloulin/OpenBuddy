/**
 * Real e2e for `pi-worktree` (G8 PR 3, plan4.1.md §9.17.9).
 *
 * Verifies that the canonical worktree package installs and exposes
 * a loadable entry. Listed in
 * `electron/main/agent/pi-extension-discovery.ts:55`.
 */
...
describe("canonical-pi: pi-worktree real install (G8 PR 3)", () => {
  let result: InstallResult;

  beforeAll(() => {
    result = tryInstallCanonicalPiPackage("pi-worktree", 120_000);
  }, 120_000);
  ...
  it("exposes a package.json with name and version", () => {
    if (result.specOnly) return;
    const meta = inspectInstalledPackage(result.cwd, "pi-worktree");
    expect(meta.hasNodeModules).toBe(true);
    expect(meta.name).toBe("pi-worktree");
