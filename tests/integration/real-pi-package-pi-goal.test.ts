/**
 * Real e2e for `pi-goal` (G8 PR 1, plan4.1.md §9.16).
 *
 * Listed in `electron/main/agent/pi-extension-discovery.ts:42`.
 */
...
describe("canonical-pi: pi-goal real install (G8 PR 1)", () => {
  let result: InstallResult;

  beforeAll(() => {
    result = tryInstallCanonicalPiPackage("pi-goal", 120_000);
  }, 120_000);
  ...
  it("exposes a package.json with name and version", () => {
    if (result.specOnly) return;
    const meta = inspectInstalledPackage(result.cwd, "pi-goal");
    expect(meta.hasNodeModules).toBe(true);
    expect(meta.name).toBe("pi-goal");
