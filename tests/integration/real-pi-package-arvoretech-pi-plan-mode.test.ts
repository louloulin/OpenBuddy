/**
 * Real e2e for `@arvoretech/pi-plan-mode` (G8 PR 3, plan4.1.md §9.17.9).
 *
 * Verifies that the canonical arvoretech plan-mode package installs and exposes
 * a loadable entry. Listed in
 * `electron/main/agent/pi-extension-discovery.ts:33`.
 */
...
describe("canonical-pi: @arvoretech/pi-plan-mode real install (G8 PR 3)", () => {
  let result: InstallResult;

  beforeAll(() => {
    result = tryInstallCanonicalPiPackage("@arvoretech/pi-plan-mode", 120_000);
  }, 120_000);
  ...
  it("exposes a package.json with name and version", () => {
    if (result.specOnly) return;
    const meta = inspectInstalledPackage(result.cwd, "@arvoretech/pi-plan-mode");
    expect(meta.hasNodeModules).toBe(true);
    expect(meta.name).toBe("@arvoretech/pi-plan-mode");
