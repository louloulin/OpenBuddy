/**
 * Real e2e for `pi-simplify` (G8 PR 3, plan4.1.md §9.17.9).
 *
 * Verifies that the canonical simplify package installs and exposes
 * a loadable entry. Listed in
 * `electron/main/agent/pi-extension-discovery.ts:53`.
 */
...
describe("canonical-pi: pi-simplify real install (G8 PR 3)", () => {
  let result: InstallResult;

  beforeAll(() => {
    result = tryInstallCanonicalPiPackage("pi-simplify", 120_000);
  }, 120_000);
  ...
  it("exposes a package.json with name and version", () => {
    if (result.specOnly) return;
    const meta = inspectInstalledPackage(result.cwd, "pi-simplify");
    expect(meta.hasNodeModules).toBe(true);
    expect(meta.name).toBe("pi-simplify");
