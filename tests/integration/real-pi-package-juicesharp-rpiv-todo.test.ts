/**
 * Real e2e for `@juicesharp/rpiv-todo` (G8 PR 3, plan4.1.md §9.17.9).
 *
 * Verifies that the canonical juicesharp rpiv-todo package installs and exposes
 * a loadable entry. Listed in
 * `electron/main/agent/pi-extension-discovery.ts:24`.
 */
...
describe("canonical-pi: @juicesharp/rpiv-todo real install (G8 PR 3)", () => {
  let result: InstallResult;

  beforeAll(() => {
    result = tryInstallCanonicalPiPackage("@juicesharp/rpiv-todo", 120_000);
  }, 120_000);
  ...
  it("exposes a package.json with name and version", () => {
    if (result.specOnly) return;
    const meta = inspectInstalledPackage(result.cwd, "@juicesharp/rpiv-todo");
    expect(meta.hasNodeModules).toBe(true);
    expect(meta.name).toBe("@juicesharp/rpiv-todo");
