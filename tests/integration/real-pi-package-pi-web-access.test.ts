/**
 * Real e2e for `pi-web-access` (G8 PR 2, plan4.1.md §9.16.8).
 *
 * Verifies that the canonical web-access package installs and exposes
 * a loadable entry. Listed in
 * `electron/main/agent/pi-extension-discovery.ts:27`.
 */
...
describe("canonical-pi: pi-web-access real install (G8 PR 2)", () => {
  let result: InstallResult;

  beforeAll(() => {
    result = tryInstallCanonicalPiPackage("pi-web-access", 120_000);
  }, 120_000);
  ...
  it("exposes a package.json with name and version", () => {
    if (result.specOnly) return;
    const meta = inspectInstalledPackage(result.cwd, "pi-web-access");
    expect(meta.hasNodeModules).toBe(true);
    expect(meta.name).toBe("pi-web-access");
