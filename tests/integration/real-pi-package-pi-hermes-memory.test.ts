/**
 * Real e2e for `pi-hermes-memory` (G8 PR 2, plan4.1.md §9.16.8).
 *
 * Verifies that the canonical Hermes memory package installs and exposes
 * a loadable entry. Listed in
 * `electron/main/agent/pi-extension-discovery.ts:22`.
 */
...
describe("canonical-pi: pi-hermes-memory real install (G8 PR 2)", () => {
  let result: InstallResult;

  beforeAll(() => {
    result = tryInstallCanonicalPiPackage("pi-hermes-memory", 120_000);
  }, 120_000);
  ...
  it("exposes a package.json with name and version", () => {
    if (result.specOnly) return;
    const meta = inspectInstalledPackage(result.cwd, "pi-hermes-memory");
    expect(meta.hasNodeModules).toBe(true);
    expect(meta.name).toBe("pi-hermes-memory");
