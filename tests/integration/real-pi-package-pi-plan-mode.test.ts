/**
 * Real e2e for `pi-plan-mode` (G8 PR 1, plan4.1.md §9.16).
 *
 * Listed in `electron/main/agent/pi-extension-discovery.ts:31`.
 */
...
describe("canonical-pi: pi-plan-mode real install (G8 PR 1)", () => {
  let result: InstallResult;

  beforeAll(() => {
    result = tryInstallCanonicalPiPackage("pi-plan-mode", 120_000);
  }, 120_000);
  ...
  it("exposes a package.json with name and version", () => {
    if (result.specOnly) return;
    const meta = inspectInstalledPackage(result.cwd, "pi-plan-mode");
    expect(meta.hasNodeModules).toBe(true);
    expect(meta.name).toBe("pi-plan-mode");
