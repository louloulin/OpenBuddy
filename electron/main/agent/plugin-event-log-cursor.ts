export type PluginEventLogCursorErrorCode = "unknown-session" | "already-attached" | "not-attached" | "frozen" | "cleared" | "generation-mismatch";
...
type CursorRecord = { generation: number; frozen: boolean; archived: boolean; cursors: Map<string, string | null> };
...
export class PluginEventLogCursorRegistry {
  ...
  attachSurface(sessionId: string, surfaceId: string): PluginEventLogCursorResult {
    ...
    const active = record ?? { generation: 1, frozen: false, archived: false, cursors: new Map<string, string | null>() };
