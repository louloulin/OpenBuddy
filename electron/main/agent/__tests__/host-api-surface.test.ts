...
//
// 2026-09 additions (reviewed, both backed by real host-module impls wired
// in agent-host.ts — not stubs):
//   - pluginEventLogCursor  → host-modules/plugin-state, replay cursor query
//   - updateExtensionPolicy → host-modules/plugin-mutations, applies
//     allow/deny overrides and reloads the live Pi extension host
const STABLE_SURFACE = new Set(
  "abort abortBash abortRetry archiveWorkspaceSession authorizeMcp authStatus bindCurrentSessionToTenant cancelMcpAuthorization clearSessionMetadata compact createWorkspace currentAgentPreset deepSeekCordisSnapshot deepSeekPiBridgeDescription deleteModel deleteProvider deleteSession deleteWorkspace dispose ensureNewSession ensureTypertReady extensionsBound followUp forkSession getAvailableThinkingLevels getCompactionSettings getContext getCwd getHarnessResumeToken getHarnessSessionCursors getModel getModelRuntime getSession getSessionStats getSessionTree getStoredPluginState getToolRegistry init insertWorkspaceBefore insertWorkspaceSessionBefore inspirationGenerate installDefaultPiPackages installProfileBundle interruptSubagent invokeConnection invokeDeepSeekCordis invokeRemote killTask listActivePluginTransactions listAgentPresets listCommands listPlugins listProfileRemoteContributions listRendererPluginEntries listRunningTasks listSessionJobs listSessions listSkills listSubagentChildren listTools listWorkspaces loadSession mcpCapabilityGovernance mcpStatus newSession onEvent onPluginEvent pluginEventLogCursor pluginEvents pluginInventory pluginReadiness pluginSnapshot profilePackages prompt promptContent promptSubagent providerCatalog readSessionAttachment readSessionEntries registerRemote reloadMcp reloadPiExtensions reloadPiRuntime reloadPlugin removeProfileBundle renameSession renameWorkspace rendererPluginBootGraph reportActivePluginTransaction resetPluginState resolveRendererPluginModule resolveUiRequest resourceInventory rewindSession saveModel saveProvider selectAgentPreset sessionBaselines sessionFile sessionInfo sessionProjectionBaseline sessionUsage setAllArchived setAutoCompaction setAutoRetry setHarnessResumeToken setHarnessSessionCursors setFollowUpMode setModel setPluginEnabled setSessionArchived setSessionExpert setSessionPinned setSteeringMode setThinkingLevel steer subagentHistory syncWorkbenchScope unregisterRemote updateExtensionPolicy updatePluginConfig updateSessionQueue waitUntilReady"
    .split(/\s+/)
    .filter(Boolean),
);
