 ...
 * This entire module (445 LOC) will be deleted in Phase L.3 of the v7 plan.
 * K.2 (commit 27f0280) replaced the manifest serialization entry point with
 * `openbuddy-plugin-manifest.ts` + `serializeHarnessTrack`; L.3 removes this
 * file and migrates the 14 call sites to PI `loadExtensions()` directly.
 *
 * Marking with @deprecated so ESLint and code review surface the migration
 * targets. Remove this header and the file in the L.3 commit.
 */
