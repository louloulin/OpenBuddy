...
// Lazy: @openbuddy/capability-email is a heavy capability chunk (≈330KB
// pre-bundle). Loaded on the first email:* IPC call and cached for the
// rest of the process lifetime so each click does not re-pay the
// microtask + Promise resolution cost of `await import(...)`.
import type * as CapabilityEmail from "@openbuddy/capability-email";

type EmailHandlers = typeof CapabilityEmail.emailHandlers;
let emailHandlersPromise: Promise<EmailHandlers> | null = null;
function loadEmailHandlers(): Promise<EmailHandlers> {
	if (emailHandlersPromise === null) {
		emailHandlersPromise = import("@openbuddy/capability-email").then(
			(module) => module.emailHandlers,
		);
	}
	return emailHandlersPromise;
}
...
export function registerEmailIpc(getWindow: () => BrowserWindow | null): void {
	const currentWindow = () => getWindow();
		...
		ipcMain.handle("email:action-center-query", async (_e, args?: unknown) => {
			...
			return (await loadEmailHandlers()).actionCenterQuery({
				...(input.accountId === undefined ? {} : { accountId: requiredString(input.accountId, "accountId") }),
		...
		ipcMain.handle("email:save-analysis", async (_e, args: unknown) => {
			...
			return (await loadEmailHandlers()).saveAnalysis({
