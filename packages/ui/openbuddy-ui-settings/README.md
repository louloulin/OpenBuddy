...
- **命名**: `@openbuddy/ui-settings`,目录 `packages/ui/openbuddy-ui-settings/`
...
- 值/组件: `HomePage` ← `./HomePage`
- 值/组件: `SettingsPanel` ← `./SettingsPanel`
- 值/组件: `AssistantsPanel` ← `./AssistantsPanel`
- 值/组件: `PolicySettingsPanel` ← `./PolicySettingsPanel`
- 多行导出: 10 项 — PersonalizeSettingsPanel, ShortcutsSettingsPanel, HelpSettingsPanel, SecuritySettingsPanel …

> 完整签名见 `src/index.ts`。子路径导入:

- `@openbuddy/ui-settings/client` — 槽位注册入口,由 `@openbuddy/ui-runtime` 调用
- `@openbuddy/ui-settings/invariant` — 不变式同伴(开发态类型守卫)
