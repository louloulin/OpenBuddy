...
- **命名**: `@openbuddy/ui-email`,目录 `packages/ui/openbuddy-ui-email/`
...
- 值/组件: `EmailComposer` ← `./EmailComposer`
- 值/组件: `EmailPanel` ← `./EmailPanel`

> 完整签名见 `src/index.ts`。子路径导入:

- `@openbuddy/ui-email/client` — 槽位注册入口,由 `@openbuddy/ui-runtime` 调用
- `@openbuddy/ui-email/invariant` — 不变式同伴(开发态类型守卫)
