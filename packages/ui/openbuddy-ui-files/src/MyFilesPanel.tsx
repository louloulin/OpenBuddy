...
import { formatFileSize, inferFileTypeFromExt, relativeTime, type FileType, type LocalFileItem } from "@/lib/files/file-utils";
...
function ArtifactsTab({
  ...
  const [entries, setEntries] = useState<LocalFileItem[]>([]);
...
function LocalFilesTab({
  ...
  const loadDir = useCallback(async (dir: string) => {
    ...
    setLoading(true);
    try {
      const { invoke } = await import("@/lib/platform/electron-api");
      const entries: Array<{
        name: string;
        path: string;
        is_dir: boolean;
      ...
      setFiles([]);
      onToast?.("该功能需要 browse_directory Electron-compatible 命令支持");
    } finally {
      setLoading(false);
    }
  }, [onToast]);
