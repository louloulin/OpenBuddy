...
- **命名**: `@openbuddy/ui-home`,目录 `packages/ui/openbuddy-ui-home/`
...
- 值/组件: `HomeHeader` ← `./HomeHeader`
- 值/组件: `HomeComposer` ← `./HomeComposer`
- 值/组件: `SceneTabs` ← `./SceneTabs`
- 值/组件: `PracticeCases` ← `./PracticeCases`

> 完整签名见 `src/index.ts`。子路径导入:

- `@openbuddy/ui-home/client` — 槽位注册入口,由 `@openbuddy/ui-runtime` 调用
- `@openbuddy/ui-home/invariant` — 不变式同伴(开发态类型守卫)
