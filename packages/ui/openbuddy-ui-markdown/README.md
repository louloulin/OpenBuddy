...
- **命名**: `@openbuddy/ui-markdown`,目录 `packages/ui/openbuddy-ui-markdown/`
...
- 值/组件: `Markdown` ← `./components/Markdown`
- 值/组件: `MarkdownInlineCode` ← `./components/MarkdownInlineCode`
- 值/组件: `MarkdownPre` ← `./components/MarkdownPre`
- 值/组件: `MarkdownPreMermaid` ← `./components/MarkdownPreMermaid`
- 值/组件: `CodeBlockActions` ← `./components/CodeBlockActions`
- 类型契约: `MarkdownProps` ← `./components/Markdown`
- 值/组件: `preprocessMarkdown` ← `./components/preprocess`

> 完整签名见 `src/index.ts`。子路径导入:

- `@openbuddy/ui-markdown/client` — 槽位注册入口,由 `@openbuddy/ui-runtime` 调用
- `@openbuddy/ui-markdown/invariant` — 不变式同伴(开发态类型守卫)
