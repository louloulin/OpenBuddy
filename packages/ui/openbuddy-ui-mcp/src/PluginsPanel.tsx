/**
 * 插件面板 - 对接 pi 的 x.ai/plugins/list + x.ai/plugins/action
 *
...
export function PluginsPanel({ sessionId, onToast }: PluginsPanelProps) {
  ...
  const [reloadingPi, setReloadingPi] = useState(false);
  ...
  const handleReloadPi = useCallback(async () => {
    setReloadingPi(true);
    try {
      ...
      onToast?.(`重载 Pi 扩展失败：${String(e).replace(/^Error:\s*/, "")}`);
    } finally {
      setReloadingPi(false);
    }
  }, [onToast, reload]);
  ...
  return (
    <div className="plugins-panel">
      {/* UX-3: clarify the relationship between this panel (legacy pi
          x.ai/plugins enable/disable) and the OpenBuddyPluginPanel
          (OpenBuddy plugin host + Cordis services + pi
          profile.piExtensions). Without this banner users saw two
          side-by-side plugin panels and did not know which to use when. */}
      <div className="plugins-panel__cross-link" role="note">
        <span className="plugins-panel__cross-link-icon" aria-hidden="true">π</span>
        <span className="plugins-panel__cross-link-text">
          本面板展示 legacy pi <code>x.ai/plugins</code> 启停状态（轻量列表 + enable/disable）。
          OpenBuddy 自家 plugin host + Cordis 服务请切到「OpenBuddy 插件」标签，那里才是完整视图。
        </span>
      </div>
      <div className="plugins-panel__header">
        ...
        <button
          className="plugins-panel__action-btn"
          onClick={handleReloadPi}
          disabled={reloadingPi}
          title="重新加载当前 Pi 扩展"
        >
          <RefreshCwIcon size="sm" /> {reloadingPi ? "重载中…" : "重载 Pi"}
        </button>
      </div>
