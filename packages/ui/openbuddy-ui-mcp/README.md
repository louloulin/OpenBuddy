...
- **命名**: `@openbuddy/ui-mcp`,目录 `packages/ui/openbuddy-ui-mcp/`
...
- 值/组件: `DiscoverPanel` ← `./DiscoverPanel`
- 值/组件: `MarketplacePanel` ← `./MarketplacePanel`
- 值/组件: `NotifyChannelsPanel` ← `./NotifyChannelsPanel`
- 值/组件: `OpenBuddyPluginPanel` ← `./OpenBuddyPluginPanel`
- 值/组件: `PluginsPanel` ← `./PluginsPanel`
- 值/组件: `ResourceCatalogPanel` ← `./ResourceCatalogPanel`

> 完整签名见 `src/index.ts`。子路径导入:

- `@openbuddy/ui-mcp/client` — 槽位注册入口,由 `@openbuddy/ui-runtime` 调用
- `@openbuddy/ui-mcp/invariant` — 不变式同伴(开发态类型守卫)
