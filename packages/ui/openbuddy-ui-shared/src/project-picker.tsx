...
export function useLiveProjectPickerOptions(): Record<"connectors" | "experts" | "skills", RefItem[]> {
  const [options, setOptions] = useState<Record<"connectors" | "experts" | "skills", RefItem[]>>({
    connectors: [],
    experts: [],
    skills: [],
  });
  useEffect(() => {
    ...
    void Promise.allSettled([
      connectorsDefaultRoot().then((root) => connectorsLoad(root || undefined)),
      expertsDefaultRoot().then((root) => expertsLoad(root || undefined)),
      skillsCatalogDefaultRoot().then((root) => skillsCatalogLoad(root || undefined)),
    ]).then(([connectors, experts, skills]) => {
      if (cancelled) return;
      setOptions({
        connectors: connectors.status === "fulfilled" ? connectors.value.connectors.map((item) => ({ id: item.id, name: item.name })) : [],
        experts: experts.status === "fulfilled" ? experts.value.experts.map((item) => ({ id: item.id, name: item.name })) : [],
        skills: skills.status === "fulfilled" ? skills.value.skills.map((item) => ({ id: item.id, name: item.name })) : [],
      });
    });
