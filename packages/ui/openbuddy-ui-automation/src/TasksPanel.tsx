/**
 * 运行中任务面板 - 显示 pi 的后台任务和子代理
 *
 * pi 通过工具调用启动的 background tasks 和 spawn_subagent 在后台运行。
 * 这里通过 `x.ai/task/list` + `x.ai/subagent/list_running` 观测它们，
 * 通过 `x.ai/task/kill` / `x.ai/subagent/cancel` 取消。
 *
 * 这个面板嵌入到 ChatView 右侧或作为浮层。监听 `pi://task-update` 自动刷新。
 */
...
import { taskKill, tasksList } from "@/lib/agent/pi-client";
...
export interface TasksPanelProps {
   ...
   *  that increments on each `pi://task-update`. */
...
export function TasksPanel({ refreshSignal, onToast }: TasksPanelProps) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      // pi may not support task/list — show empty.
      setTasks([]);
    } finally {
      setLoading(false);
    }
