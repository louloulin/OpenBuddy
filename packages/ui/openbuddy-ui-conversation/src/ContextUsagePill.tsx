 ...
 * Self-contained: fetches pi x.ai/session/info+usage on mount and after
...
function buildRows(ctx: ContextInfo): CategoryRow[] {
  ...
  // 守恒收敛(参照 WorkBuddy distributeWithConservation):pi 的各项是独立
...
export function ContextUsagePill({ sessionId, onRefreshSignal }: { sessionId: string; onRefreshSignal?: number }) {
  ...
  const refresh = useCallback(async () => {
    try {
      const info = await piSessionInfo(sessionId);
      if (info?.context?.total) {
        let usage: SessionUsage | undefined;
        try {
          const fetchedUsage = await piSessionUsage(sessionId);
          if (fetchedUsage) usage = fetchedUsage;
        } catch { /* nice-to-have */ }
