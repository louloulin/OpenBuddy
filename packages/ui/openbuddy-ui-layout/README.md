...
- **命名**: `@openbuddy/ui-layout`,目录 `packages/ui/openbuddy-ui-layout/`
...
- 值/组件: `AppFrame` ← `./client/AppFrame`
- 类型契约: `AppFrameProps` ← `./client/AppFrame`

> 完整签名见 `src/index.ts`。子路径导入:

- `@openbuddy/ui-layout/client` — 槽位注册入口,由 `@openbuddy/ui-runtime` 调用
- `@openbuddy/ui-layout/invariant` — 不变式同伴(开发态类型守卫)
