...
> 协作面板层。包含项目管理面板与子代理面板,用于多人多代理协同。

## 角色与边界

本包是 OpenBuddy UI 插件体系下的一个独立子包,遵循 `packages/ui/AGENTS.md` 中的约束:

- **命名**: `@openbuddy/ui-collaboration`,目录 `packages/ui/openbuddy-ui-collaboration/`
...
- 值/组件: `ProjectsPanel` ← `./ProjectsPanel`
- 值/组件: `SubagentPanel` ← `./SubagentPanel`

> 完整签名见 `src/index.ts`。子路径导入:

- `@openbuddy/ui-collaboration/client` — 槽位注册入口,由 `@openbuddy/ui-runtime` 调用
- `@openbuddy/ui-collaboration/invariant` — 不变式同伴(开发态类型守卫)
