...
import { AddIcon, SearchIcon, MoreDotsIcon, ChevronDownIcon } from "@openbuddy/ui-primitives/icons";
...
interface ProjectsPanelProps {
  ...
  /** Start a new conversation within a project (creates a real pi session). */
...
export function ProjectsPanel({ onToast, onStartProjectConversation, onNavigate }: ProjectsPanelProps) {
  ...
  const setProjectScope = useProjectsStore((s) => s.setScope);
  const replaceEnterpriseProjects = useProjectsStore((s) => s.replaceEnterpriseProjects);
  const [enterpriseContext, setEnterpriseContext] = useState<{ tenantId: string; subject: string } | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    const syncScope = async (status: CasdoorSessionView) => {
      if (cancelled) return;
      const tenantId = status.tenantContext.activeTenantId;
      const subject = status.identity?.subject;
      const context = status.status === "signed_in" && tenantId && subject ? { tenantId, subject } : null;
      setEnterpriseContext(context);
      setProjectScope(context ? `tenant:${tenantId}:subject:${subject}` : "local");
      if (context) await casdoorListResources("project").then(replaceEnterpriseProjects).catch(() => undefined);
      if (!cancelled) setScopeReady(true);
    };
    void (async () => {
      try {
        await syncScope(await casdoorStatus());
      } catch {
        if (!cancelled) {
          setEnterpriseContext(null);
          setProjectScope("local");
          setScopeReady(true);
        }
      }
    })();
    ...
    void listen<CasdoorSessionView>("casdoor://auth", (event) => {
      setScopeReady(false);
      void syncScope(event.payload).catch(() => {
        if (!cancelled) {
          setEnterpriseContext(null);
          setProjectScope("local");
          setScopeReady(true);
  ...
  }, [replaceEnterpriseProjects, setProjectScope]);
  ...
  if (!scopeReady) return <div className="project-grid-empty">正在加载企业项目范围…</div>;
    ...
    <div className="project-page">
      ...
      {deleteTarget && (
        <ProjectConfirmDialog
          title="删除项目"
          message={`确定删除项目「${deleteTarget.name}」？项目下的本地配置和记录也会从 OpenBuddy 中移除。`}
...
function CreateProjectDialog({
  ...
  const [enterprise, setEnterprise] = useState(Boolean(enterpriseContext));
  const [encryptAtRest, setEncryptAtRest] = useState(false);
  const [busy, setBusy] = useState(false);
  const createIdempotencyKey = useRef(`project-create:${Date.now()}:${Math.random().toString(36).slice(2, 10)}`);
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      const resource = enterprise && enterpriseContext
        ? await casdoorCreateResource({
            type: "project",
            name: name.trim(),
            idempotencyKey: createIdempotencyKey.current,
            metadata: { ...(encryptAtRest ? { encryptedBy: "casdoor" } : {}), templateId: templateId ?? "custom", instructionLength: instructions.length },
          })
        : undefined;
      const saved = add({
        name: name.trim(), templateId, instructions: instructions.trim() || undefined, connectors, experts, skills,
        ...(resource ? { enterpriseResourceId: resource.id, enterpriseVersion: resource.version, tenantId: resource.tenantId } : {}),
      });
      onConfirm(saved);
    } catch (error) {
  ...
  return (
    <div className="modal-overlay create-colleague-overlay" onClick={onCancel}>
      <div className="create-colleague-dialog create-project-dialog" onClick={(e) => e.stopPropagation()} role="dialog">
        ...
        <label className="create-colleague-field create-colleague-field--inline" data-testid="casdoor-encrypt-toggle">
          <input type="checkbox" checked={encryptAtRest} onChange={(e) => setEncryptAtRest(e.target.checked)} />
          <span>加密存储（需要网关配置 <code>RESOURCE_GATEWAY_CASDOOR_PUBLIC_KEY</code>；勾选后元数据中的字符串字段会用 Casdoor 公钥静态加密，仅 Casdoor 私钥持有者可解密）</span>
        </label>
        ...
        <div className="create-colleague-footer create-project-footer">
          ...
          <button className="btn btn--primary" onClick={() => void submit()} disabled={!name.trim() || busy}>{busy ? "创建中…" : "确定"}</button>
        </div>
      </div>
