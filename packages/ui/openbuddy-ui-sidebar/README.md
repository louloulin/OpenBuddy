...
- **命名**: `@openbuddy/ui-sidebar`,目录 `packages/ui/openbuddy-ui-sidebar/`
...
- 值/组件: `ColleaguesPanel` ← `./ColleaguesPanel`
- 值/组件: `ConversationList` ← `./ConversationList`
- 值/组件: `PinnedSection` ← `./PinnedSection`
- 值/组件: `SkillRecommendBar` ← `./SkillRecommendBar`
- 值/组件: `WorkspaceGroup` ← `./WorkspaceGroup`
- 值/组件: `Sidebar` ← `./Sidebar`

> 完整签名见 `src/index.ts`。子路径导入:

- `@openbuddy/ui-sidebar/client` — 槽位注册入口,由 `@openbuddy/ui-runtime` 调用
- `@openbuddy/ui-sidebar/invariant` — 不变式同伴(开发态类型守卫)
