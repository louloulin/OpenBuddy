 ...
 * Lists every cwd pi has seen (from list_workspaces). Selecting one
...
export function WorkspacePicker({
  ...
  /** True while piCreateWorkspace + piListWorkspaceRegistry round-trip the IPC.
   *  R2.5 — shows an inline spinner so the user can see the click registered. */
  loading?: boolean;
}) {
      ...
      >
        ...
        {loading ? (
          <Loader2
            size={14}
            strokeWidth={1.75}
            className="workspace-picker__spinner"
            aria-label="切换中"
          />
        ) : (
