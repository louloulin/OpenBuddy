...
import { ChevronLeftIcon } from "@openbuddy/ui-primitives/icons";
import { mcpConfigPath, mcpConfigRead, mcpConfigSave } from "@/lib/agent/pi-client";
 ...
 *  saving; the backend also re-validates and mirrors entries into pi. */
export function McpConfigEditor({
  ...
  const doSave = async () => {
    ...
    setSaving(true);
    try {
      // Pass the live session (when any) so pi applies the entries now;
