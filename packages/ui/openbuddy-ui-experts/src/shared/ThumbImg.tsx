...
import { expertsThumbnail } from "@/lib/agent/pi-client";
...
function loadThumb(path: string, root?: string): Promise<string> {
  const key = `${root ?? ""}\0${path}`;
  const hit = cache.get(key);
  if (hit) return Promise.resolve(hit);
  let p = inflight.get(key);
  if (!p) {
      p = expertsThumbnail(path, root)
      .then((b64) => {
        const url = b64
...
export function ThumbImg({
  local, url, name, size = 40, shape = "square", color, className, root,
}: ThumbImgProps) {
  const [src, setSrc] = useState<string | undefined>(() =>
    local ? cached(`${root ?? ""}\0${local}`) : url,
  );
  ...
  useEffect(() => {
    ...
    const start = () => {
      ...
      loadThumb(local, root).then((u) => setSrc(u || url));
    };
