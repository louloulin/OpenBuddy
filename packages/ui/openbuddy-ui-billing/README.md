...
- **命名**: `@openbuddy/ui-billing`,目录 `packages/ui/openbuddy-ui-billing/`
...
- 值/组件: `BillingPanel` ← `./BillingPanel`
- 值/组件: `CreditPricingPanel` ← `./CreditPricingPanel`
- 值/组件: `CreditReconciliationPanel` ← `./CreditReconciliationPanel`
- 值/组件: `CreditWalletPanel` ← `./CreditWalletPanel`
- 值/组件: `UsageQuotaPanel` ← `./UsageQuotaPanel`

> 完整签名见 `src/index.ts`。子路径导入:

- `@openbuddy/ui-billing/client` — 槽位注册入口,由 `@openbuddy/ui-runtime` 调用
- `@openbuddy/ui-billing/invariant` — 不变式同伴(开发态类型守卫)
