...
- **命名**: `@openbuddy/ui-runtime`,目录 `packages/ui/openbuddy-ui-runtime/`
...
- 类型契约: `SlotCoreLike, UiPlugin, UiRuntimeContext` ← `@openbuddy/ui-slots`

> 完整签名见 `src/index.ts`。子路径导入:

- `@openbuddy/ui-runtime/client` — 槽位注册入口,由 `@openbuddy/ui-runtime` 调用
- `@openbuddy/ui-runtime/invariant` — 不变式同伴(开发态类型守卫)
