...
describe("sessions-store drafts", () => {
  ...
  it("migrateSession 原子地将 pending id 重命名为真实 id(独立分组 + 草稿 + focus 一起)", () => {
    useSessionsStore.getState().setHomeCwd("/home");
    useSessionsStore.getState().setCurrent("__pending_1");
    useSessionsStore.getState().setDraft("__pending_1", "draft text");
    useSessionsStore.getState().upsert({
      sessionId: "__pending_1",
      cwd: "/home",
      title: "新会话",
    });
    const before = useSessionsStore.getState();
    expect(before.independent.some((x) => x.sessionId === "__pending_1")).toBe(true);
    expect(before.drafts["__pending_1"]).toBe("draft text");
    expect(before.currentSessionId).toBe("__pending_1");

    useSessionsStore.getState().migrateSession("__pending_1", "real_42");

    const after = useSessionsStore.getState();
    // 1) Pending row renamed in place, no insert + delete churn
    expect(after.independent.some((x) => x.sessionId === "__pending_1")).toBe(false);
    expect(after.independent.some((x) => x.sessionId === "real_42")).toBe(true);
    expect(after.independent.length).toBe(before.independent.length);
    // 2) Draft key moved atomically
    expect(after.drafts["__pending_1"]).toBeUndefined();
    expect(after.drafts["real_42"]).toBe("draft text");
    // 3) Focus updated
    expect(after.currentSessionId).toBe("real_42");
  });

  it("migrateSession 不匹配时是 no-op", () => {
    useSessionsStore.getState().setHomeCwd("/home");
    useSessionsStore.getState().upsert({
      sessionId: "real_42",
      cwd: "/home",
    ...
    const before = useSessionsStore.getState();
    useSessionsStore.getState().migrateSession("__pending_1", "real_99");
    const after = useSessionsStore.getState();
    expect(after).toEqual(before);
  });

  it("migrateSession(oldId, oldId) 是 no-op", () => {
    useSessionsStore.getState().setHomeCwd("/home");
    useSessionsStore.getState().upsert({
      sessionId: "real_42",
      cwd: "/home",
    ...
    const before = useSessionsStore.getState();
    useSessionsStore.getState().migrateSession("real_42", "real_42");
    expect(useSessionsStore.getState()).toEqual(before);
  });
});
