...
interface SessionsState {
  ...
  /** 空间分组: one node per working directory pi has seen. */
  ...
  /** The "inbox" cwd = the directory pi started in. Sessions in this cwd
   *  form the 任务 group; every other cwd is a 空间 node. (pi rejects empty
  ...
  /** R2.5 — show archived sessions in the sidebar's 已归档 group.
   *  Default false so the live list stays scannable, but the sidebar
   *  auto-flips this to true when every session for the active cwd is
   *  archived (recovery path for an accidental bulk archive). */
  showArchived: boolean;
  /**
   * Per-session Composer drafts (unsent textarea text), keyed by sessionId.
   * UI-only state: pi has no concept of "user hasn't pressed send yet", so
  ...
  /** Re-key the draft from `oldId` to `newId`. Used by the optimistic
   *  "新建任务" flow when a pending session id is replaced with the real
   *  one -- we move the user's in-progress draft over so they don't lose
   *  what they've typed while the IPC was pending. */
  renameDraft: (oldId: string, newId: string) => void;
   ...
   *  it lives, so a cwd-less `{ sessionId, title }` (e.g. pi://summary)
  ...
  /** Atomically rename `oldId` -> `newId` across every group + draft key.
   *  Used by the optimistic "新建任务" flow: the pending sidebar row
   *  (`__pending_<nonce>`) gets rekeyed in one store mutation when the
   *  real session id arrives from piNewSession, avoiding three separate
   *  upsert/remove/upsert renders. */
...
/** Returns true when the sidebar has any archived sessions across all
 *  groups. Used to auto-expand the 已归档 group on first paint when the
 *  user lands in a state where every session is archived. */
export function selectArchivedCount(s: { independent: SessionSummary[]; workspaceSessions: Record<string, SessionSummary[]> }): number {
  let count = 0;
  for (const session of s.independent) if (session.archived === true) count++;
  for (const list of Object.values(s.workspaceSessions)) {
    for (const session of list) if (session.archived === true) count++;
  }
  return count;
}
...
export const useSessionsStore = create<SessionsState>((set) => ({
  ...
  renameDraft: (oldId, newId) =>
    set((state) => {
      if (oldId === newId) return {};
      if (!Object.prototype.hasOwnProperty.call(state.drafts, oldId)) return {};
      const next = { ...state.drafts };
      const moved = next[oldId];
      delete next[oldId];
      next[newId] = moved;
      return { drafts: next };
    }),
  ...
  upsert: (s) =>
    set((state) => {
      ...
      // pi started in (homeCwd); pi rejects empty cwd so every session has
  ...
  migrateSession: (oldId, newId) =>
    set((state) => {
      ...
      // 任务 group
      const independent = state.independent.map((entry) => {
        if (entry.sessionId !== oldId) return entry;
        touched = true;
        return { ...entry, sessionId: newId };
      });
      // 空间 group caches
      const workspaceSessions: Record<string, SessionSummary[]> = {};
      for (const cwd of Object.keys(state.workspaceSessions)) {
        workspaceSessions[cwd] = state.workspaceSessions[cwd].map((entry) => {
          if (entry.sessionId !== oldId) return entry;
          touched = true;
          return { ...entry, sessionId: newId };
        });
      ...
      // Draft key
      let drafts = state.drafts;
      if (Object.prototype.hasOwnProperty.call(state.drafts, oldId)) {
        touched = true;
        drafts = { ...state.drafts };
        const moved = drafts[oldId];
        delete drafts[oldId];
        drafts[newId] = moved;
      }
      // Focus
      ...
      if (!touched) return {};
      return { independent, workspaceSessions, drafts, currentSessionId };
    }),
}));
