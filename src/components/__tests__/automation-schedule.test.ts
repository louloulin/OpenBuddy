...
import {
  ...
  startsInLabel,
  validateDraft,
} from "@openbuddy/ui-automation/schedule-utils";
import { AUTOMATION_TEMPLATES } from "@openbuddy/ui-automation/template-config";
...
describe("schedule-utils", () => {
  ...
  it("模板字段齐备并包含邮件自动化模板", () => {
    expect(AUTOMATION_TEMPLATES.length).toBeGreaterThanOrEqual(12);
    ...
    expect(AUTOMATION_TEMPLATES.map((t: { title: string }) => t.title)).toContain("每日 AI 新闻推送");
    expect(AUTOMATION_TEMPLATES.map((t: { title: string }) => t.title)).toContain("可爱萌宠手机壁纸");
    expect(AUTOMATION_TEMPLATES.map((t: { title: string }) => t.title)).toContain("每日邮件简报");
    expect(AUTOMATION_TEMPLATES.map((t: { title: string }) => t.title)).toContain("邮件 Reply Zero 扫描");
  });
