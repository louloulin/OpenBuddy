...
import { HomePage } from "@openbuddy/ui-settings";
...
describe("HomePage", () => {
  ...
  it("working 场景下 chip 数 == COLLAPSED_VISIBLE_COUNT,默认全部展开,没有'更多'", () => {
    ...
    expect(screen.getByRole("button", { name: "数据分析及可视化" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "个人工作台" })).toBeInTheDocument();
  ...
  it("多于 COLLAPSED_VISIBLE_COUNT 的场景折叠为前 N 个 + 更多,点击更多展开全部", () => {
    render(<HomePage {...base} />);
    // 代码开发场景有 4 个 chip,等于阈值,不折叠。
    fireEvent.click(screen.getByRole("tab", { name: /代码开发/ }));
    expect(screen.getByRole("button", { name: "日常开发" })).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "更多" })).toBeNull();
  });
