...
import { ask, save } from "@/lib/platform/electron-api";
import type { MarkdownConfig, PathType } from "@openbuddy/ui-markdown/components/types";
...
/**
 * Build a MarkdownConfig wired to Electron-compatible shell/fs commands for the active session.
 */
export function createMarkdownHostConfig(
  ...
  const onLinkClick: MarkdownConfig["onLinkClick"] = ({ href, event }) => {
    if (!href) return;
    // Intercept http(s) so we open in the system browser (Electron-compatible webview
