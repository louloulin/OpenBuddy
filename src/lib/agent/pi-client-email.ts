...
export const emailConfirmProcessingPlan = (planId: string, options?: { confirmed?: boolean }) => invoke<string>("email:confirm-processing-plan", { planId, ...(options?.confirmed === undefined ? {} : { confirmed: options.confirmed }) });
...
export const emailCreateRemindersFromAnalysis = (input: EmailAnalysisReminderInput) => invoke<EmailAnalysisReminderResult>("email:create-reminders-from-analysis", { ...input, confirmed: true });
...
export const emailPrepareScheduleSend = (draftId: string, scheduledAt: string) => invoke<string>("email:prepare-schedule-send", { draftId, scheduledAt, confirmed: true });
...
export const emailPrepareSend = (draftId: string) => invoke<string>("email:prepare-send", { draftId, confirmed: true });
